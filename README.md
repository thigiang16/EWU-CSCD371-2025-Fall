...
- For this assignment, always use `Assert.AreEqual<T>()` (the generic version) ✔
  - Note: MSTest's Assert supports AreEqual<T>() for value comparisons; the test suite uses Assert.IsTrue/IsFalse for boolean checks where that is more idiomatic in MSTest.
